...
If Phase 5 touched persisted data, migrations, status/enum values, denormalized data, or data-access invariants, dispatch the **`reviewer-data-integrity`** subagent (`subagents/reviewer-data-integrity.md`) before `simplify`. The realignment is not complete until the data-integrity pass either reports no issues or every finding is fixed or explicitly surfaced.
...
Renames sometimes leave behind translation helpers, transitional aliases, or branches that handled the old vocabulary. Run `playbooks/simplify/PLAYBOOK.md` on the diff to collapse what's now dead code and catch parallel patterns the rename may have created.
