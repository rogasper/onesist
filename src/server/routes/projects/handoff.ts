...
import { readFile, scanDirectory } from "~/lib/file-router";
...
router.get("projects/:id/handoff", async ({ params, query }) => {
  ...
  // Collect context files (safe even without repo — missing files are skipped)
  const masterErd = safeRead(rootPath, "MASTER_ERD.md", 20000);
  const masterSpec = safeRead(rootPath, "MASTER_SPEC_API.md", 20000);
  const projectContext = safeRead(rootPath, "project_context.md", 10000) || safeRead(rootPath, "output/project_context.md", 10000);
