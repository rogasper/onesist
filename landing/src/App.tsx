  ...
  return (
    <section id="features" className="mx-auto max-w-[1160px] px-6 py-12">
      <div className="flex items-end justify-between gap-4">
        <h2 className="text-xl font-semibold text-zinc-900">Everything to plan — nothing to execute</h2>
        <a href={GH_REPO} target="_blank" rel="noreferrer" className="text-xs text-zinc-400 hover:text-zinc-900">Docs →</a>
      </div>
      <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map((it, i) => (
          <div key={it.k} className="glass specular relative rounded-[20px] p-5" style={{ animationDelay: `${i * 80}ms` }}>
            <div className="text-sm font-semibold text-zinc-900">{it.k}</div>
            <div className="mt-2 text-sm leading-5 text-zinc-500">{it.d}</div>
            <div className="mt-4 inline-flex text-[11px] font-mono px-2 py-1 rounded-full bg-zinc-900 text-white">{it.t}</div>
          </div>
        ))}
      </div>
      <div className="mt-4 grid md:grid-cols-3 gap-4">
        <div className="glass-subtle rounded-2xl p-4 flex items-center justify-between"><span className="text-sm text-zinc-700">RTM</span><span className="text-xs font-mono text-zinc-400">BR → FR → DS → TC</span></div>
        <div className="glass-subtle rounded-2xl p-4 flex items-center justify-between"><span className="text-sm text-zinc-700">SIT</span><span className="text-xs font-mono text-zinc-400">Chrome / Safari / iOS / Android</span></div>
        <div className="glass-subtle rounded-2xl p-4 flex items-center justify-between"><span className="text-sm text-zinc-700">Timeline</span><span className="text-xs font-mono text-zinc-400">Gantt • Critical path</span></div>
      </div>
    </section>
  );
}

// --- How it works with example outputs ---
function HowItWorks() {
  const steps = [
...
function Carousel() {
  ...
  const slides = [
    { title: "Projects — Dashboard", desc: "Open / import / multi-root. 12/page, taller cards.", color: "from-[#6d7cff]/10 to-[#a78bfa]/10", label: "Dashboard", file: "dashboard.png" },
    { title: "ERD Canvas", desc: "XYFlow + Dagre layout. DBML live, table editor.", color: "from-emerald-50 to-teal-50", label: "ERD", file: "erd.png" },
    { title: "Spec + OpenAPI", desc: "Endpoint cards, search, Swagger. Consolidate openapi.yaml.", color: "from-amber-50 to-orange-50", label: "Spec", file: "spec.png" },
    { title: "Tasks — Agentic Handoff", desc: "Context + Given-When-Then + prompts. Export zip.", color: "from-sky-50 to-indigo-50", label: "Tasks", file: "tasks.png" },
    { title: "RTM & SIT", desc: "Trace BR→TC. SIT browser matrix (5 platforms).", color: "from-pink-50 to-rose-50", label: "RTM/SIT", file: "tasks.png" },
  ];
  ...
  return (
    <section id="screens" className="mx-auto max-w-[1160px] px-6 py-8">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-zinc-900">Screens — real outputs</h3>
        <span className="text-xs text-zinc-400">Auto-play • hover to pause</span>
      </div>
  ...
  return (
    <section id="changelog" className="mx-auto max-w-[1160px] px-6 py-8">
      <div className="flex items-end justify-between">
        <h3 className="text-lg font-semibold text-zinc-900">Changelog</h3>
        <a href={`${GH_REPO}/blob/main/CHANGELOG.md`} target="_blank" rel="noreferrer" onClick={() => track("changelog-full")} className="text-xs text-zinc-400 hover:text-zinc-900">View full changelog →</a>
      </div>
      <div className="mt-4 grid gap-3">
        {entries.map((e) => (
          <div key={e.v} className="flex gap-4 rounded-2xl border border-zinc-200 bg-white p-4 hover:shadow-sm transition">
            <span className="shrink-0 text-xs font-mono px-2 py-1 rounded-full bg-zinc-900 text-white h-fit">{e.v}</span>
            <div><div className="text-sm font-medium text-zinc-900">{e.t}</div><div className="text-sm text-zinc-500">{e.d}</div></div>
          </div>
        ))}
      </div>
    </section>
